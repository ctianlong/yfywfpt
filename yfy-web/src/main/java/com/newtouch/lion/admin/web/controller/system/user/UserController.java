...
public class UserController extends AbstractController {
	...
	public String index(Model model) {
		...
		//向model中添加用户所在部门中文名，主要在用户为部门限定角色时显示
		//注意懒加载异常，在web应用加载时，在UserRealm获取user时应该把department具体查出来，再放到userInfo中，这样才能保证取到department中的各种属性，不然只能取到ID
		model.addAttribute("departmentName", LoginSecurityUtil.getUser().getDepartment().getNameZh());
	...
	public DataTable<User> lists(@ModelAttribute("queryDt")QueryDt queryDt,@ModelAttribute("user") UserVo userVo) {
		...
		if (StringUtils.isNotEmpty(queryDt.getSort()) && StringUtils.isNotEmpty(queryDt.getOrder())) {
			queryCriteria.setOrderField(queryDt.getSort().replace("department", "department.departmentNo"));
...
//		}
		...
		if(subject.hasRole("PersonLimitRole")){
			UserInfo userInfo = LoginSecurityUtil.getUser();
			queryCriteria.addQueryCondition("username", userInfo.getUsername());
		...
		// 查询条件 参数类型 部门ID
		//部门限定权限控制
		if(subject.hasRole("DepartmentLimitRole")){
			UserInfo userInfo = LoginSecurityUtil.getUser();
			queryCriteria.addQueryCondition("departmentId", userInfo.getDepartment().getId());
		} else if(userVo.getDepartmentId() != null) {
				queryCriteria.addQueryCondition("departmentId", userVo.getDepartmentId());
		}
